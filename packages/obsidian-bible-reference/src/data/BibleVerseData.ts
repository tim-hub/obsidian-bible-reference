/**
 * Number of verses in each chapter for each book of the Bible (KJV/Standard).
 * Index starts from 0 (Genesis), each element is an array of verse counts for each chapter.
 */
export const BIBLE_VERSE_COUNTS: { [book: string]: number[] } = {
  Genesis: [
  ...
  '1 Samuel': [
    ...
    42, 15, 23, 28, 22, 44, 25, 12, 25, 11, 31, 13,
  ],
  '2 Samuel': [
...
const BIBLE_VERSE_COUNTS_LOWER: { [book: string]: number[] } = {}
Object.keys(BIBLE_VERSE_COUNTS).forEach((key) => {
  BIBLE_VERSE_COUNTS_LOWER[key.toLowerCase()] = BIBLE_VERSE_COUNTS[key]
})
 ...
 * @returns Number of verses, or 0 if not found
 */
export const getVerseCount = (
  bookName: string,
  chapterNumber: number
): number => {
  const counts = BIBLE_VERSE_COUNTS_LOWER[bookName.toLowerCase()]
 ...
 * @param bookName Standard book name
 * @returns Number of chapters, or 0 if not found
 */
